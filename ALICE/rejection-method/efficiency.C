...
void draw_spec(TH1D * pi0spectrum, TH1D * pi0spectrum_both, TH1D * pi0spectrum_true, TH1D* pi0position, TH1D* pi0position_both, TH1D* pi0position_true,
               TH1D* pi0width, TH1D* pi0width_both, TH1D* pi0width_true, TString name, Double_t events, TH1D* _hf_eff, Double_t events_eff);
TH1D* fill_hist(TH2D* _h, Int_t bin1, Int_t bin2, TString name, TString hist_name, int i);
void beauty_draw(TH1D* ( &h )[32][3], int i, TString name, TString hist_name);
...
TH2D *_h_true=(TH2D*)_l->FindObject("True_pi0_Inv_mass");
...
TString hist_name;
    ...
    TH1D* signal_bg_true = new TH1D("signal/background bg True","sg/bg True", bins, pTbins);
    TH1D* signal_all = new TH1D("signal all ","sg all", bins, pTbins);
    TH1D* signal_both = new TH1D("signal Disp&CPV","sg Disp&CPV", bins, pTbins);
    TH1D* signal_true = new TH1D("signal True","sg True", bins, pTbins);
    ...
    TH1D* pi0_spectrum_true = new TH1D("pi0_spectrum_true","pi0 spectrum true", bins, pTbins);
    ...
    TH1D* position_true = new TH1D("pi0position True","pi0 position True", bins, pTbins);//11
    TH1D* width_true = new TH1D("pi0width True", "pi0 width True", bins, pTbins);//watch the number of bins!
...
for(int i=1; i<bins-1; i++)
    {
      ...
      bin2 = (Int_t) _h_all->GetYaxis()->FindBin(pTbins[i+1]);
        ...
        IntFit(_hf_true, "true", signal_true, signal_bg_true, dpt, i, pi0_spectrum_true, position_true, width_true);
...
draw_spec(signal_all, signal_both, signal_true, position, position_both, position_true,
          width, width_both, width_true, "all and disp&cpv and true", events, _h_eff_mod, events1);

}

void draw_spec(TH1D * pi0spectrum, TH1D * pi0spectrum_both, TH1D * pi0spectrum_true, TH1D* pi0position, TH1D* pi0position_both,
               TH1D* pi0position_true, TH1D* pi0width, TH1D* pi0width_both, TH1D* pi0width_true, TString name, Double_t events, TH1D* _hf_eff, Double_t events1)
{
//  TCanvas *c2 = new TCanvas("c2_results","The Fit Canvas",1200,800);

  //  TCanvas *c3 = new TCanvas("Spectrum pi0 all true both","Spectrum pi0 all true both",1200,800);
  //  c3->Divide(1,3);
  ...
  //  c3->cd(1)->SetLogy();
    ...
    pi0spectrum_true->Scale(1./events);

    TH1D* cp_spec = (TH1D*) pi0spectrum->Clone();
    TH1D* cp_spec_both = (TH1D*) pi0spectrum_both->Clone();
    TH1D* cp_spec_true = (TH1D*) pi0spectrum_true->Clone();// ()посмотреть, надо ли ещё их отскалировать)
...
/*
    ...
    pi0spectrum_true->SetLineColor(kRed);
    pi0spectrum_true->SetMarkerColor(kRed);
    pi0spectrum_true->SetMarkerStyle(30);
    pi0spectrum_true->Draw("p e same");
  ...
  //  TF1  *f4  = new TF1("f4","[0]", 0.5, 10);
    ...
    pi0position_true->SetLineColor(kGreen);
    pi0position_true->SetMarkerStyle(22);
    pi0position_true->SetMarkerColor(kRed);
    pi0position_true->Draw("p e same");
...
//  f5->SetParameter(1,0.008);
  ...
  //  pi0width->Fit("f5", "RMS");
    ...
    pi0width_true->SetMarkerStyle(30);
    pi0width_true->SetLineColor(kGreen);
    pi0width_true->SetMarkerColor(kGreen);
  //  pi0width_true->SetMarkerStyle(30);
...
*/
    ...
    pi0spectrum_true->Divide(_hf_eff);

    pi0spectrum->GetXaxis()->SetTitle("p_{T}, GeV/c");
    pi0spectrum->GetYaxis()->SetTitle("dN/dp_{T}");
    pi0spectrum->SetLineColor(kBlack);
    pi0spectrum->SetMarkerColor(kBlack);
    ...
    pi0spectrum_true->SetLineColor(kRed);
    pi0spectrum_true->SetMarkerColor(kRed);
    pi0spectrum_true->GetXaxis()->SetLimits(0., 20.);
    pi0spectrum_true->SetMarkerStyle(22);

     TF1 *f_pi0 = new TF1("f_pi0","[0]+[1]*log(x)", 0.5, 19.);
  //  TF1 *f_pi0 = new TF1("f_pi0","[0]+[1]*x+[2]*pow(x, 2)+[3]*pow(x, 3)", 0.5, 19.);
    f_pi0->SetLineColor(kMagenta);
    pi0spectrum->Fit("f_pi0", "RMS+");
    pi0spectrum_both->Fit("f_pi0", "RMS+");
    pi0spectrum_true->Fit("f_pi0", "RMS+");

    //TF1 *f_pi0_both = new TF1("fpi0","[0]+[1]*x+[2]*pow(x,2)+3*pow(x, 4)", 0., 20.);
    //pi0spectrum->Fit("f6", "RMS+");


    TLegend * leg1 = new TLegend(0.15,0.53,0.47,0.8) ;
    leg1->AddEntry(pi0spectrum, "all", "lep");
    leg1->AddEntry(pi0spectrum_both, "Disp&CPV", "lep");
    leg1->AddEntry(pi0spectrum_true, "True", "lep");

    pi0spectrum->Draw("p e1");
    pi0spectrum_both->Draw("p e1 same");
    pi0spectrum_true->Draw("p e1 same");
    leg1->Draw("p e same");
    ...
    cp_spec->Divide(pi0spectrum);
    cp_spec_both->Divide(pi0spectrum_both);
    cp_spec_true->Divide(pi0spectrum_true);

    cp_spec->GetXaxis()->SetTitle("p_{T}, GeV/c");
    cp_spec->GetYaxis()->SetTitle("dN/dp_{T}");

    cp_spec->GetYaxis()->SetRangeUser(1e-10, 1.);

    cp_spec->SetLineColor(kBlack);
    cp_spec->SetMarkerColor(kBlack);
    cp_spec->GetXaxis()->SetLimits(0., 20.);
    cp_spec->SetMarkerStyle(21);

    cp_spec_both->SetLineColor(kBlue);
    cp_spec_both->SetMarkerColor(kBlue);
    cp_spec_both->GetXaxis()->SetLimits(0., 20.);
    cp_spec_both->SetMarkerStyle(20);


    cp_spec_true->SetLineColor(kRed);
    cp_spec_true->SetMarkerColor(kRed);
    cp_spec_true->GetXaxis()->SetLimits(0., 20.);
    cp_spec_true->SetMarkerStyle(22);
    ...
    TLegend * leg2 = new TLegend(0.13,0.15,0.4,0.4) ;
    leg2->AddEntry(cp_spec, "all", "lep");
    leg2->AddEntry(cp_spec_both, "Disp&CPV", "lep");
    leg2->AddEntry(cp_spec_true, "True", "lep");
  //  leg2->AddEntry(_hf_eff, "Generated", "lep");

    cp_spec->Draw("p e");
    cp_spec_both->Draw("p e same");
    cp_spec_true->Draw("p e same");
  //  _hf_eff->Draw("p e same");
    leg2->Draw("p e same");

    c->SaveAs("/home/hellaweas/alice/AliPhysics/PWGGA/PHOSTasks/PHOS_Resonances/for_efficiency/pi0/corrected_spectra.pdf");
 }
...
/*
  ...
  */  box->Draw() ;
    ...
    t3.Draw() ;
    c1DP->SaveAs("pi0/model/"+name+Form("_%d",i)+".pdf");
}
