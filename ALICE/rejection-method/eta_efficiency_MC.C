...
void draw_spec(TH1D * true_h, TH1D * true_cut, TH1D * all, TH1D * all_cut, TH1D * both, TH1D * both_cut, Double_t events, TH1D* _h_gen);
...
TFile *_file0 = TFile::Open("/home/hellaweas/alice/AliPhysics/PWGGA/PHOSTasks/PHOS_Resonances/histos.root");//watch the name!
...
TH1D* _h_gen = (TH1D*)_l->FindObject("EtaGeneratedSpectra");
...
TH1D* etaspectrum_true = new TH1D("etaspectrum_true","eta spectrum true", bins, pTbins);
TH1D* etaspectrum_true_cut = new TH1D("etaspectrum_true cut","eta spectrum true cut", bins, pTbins);
...
void draw_spec(TH1D * true_h, TH1D * true_cut, TH1D * all, TH1D * all_cut, TH1D * both, TH1D * both_cut, Double_t events, TH1D* _h_gen){
  TCanvas *c2 = new TCanvas("efficiency_MC","efficiency_MC",1200,800);
...
//  cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@Number of bins Y - "<<true_h->GetNbinsY()<<endl;
  ...
  eff_true->GetYaxis()->SetRangeUser(0., 0.2);
