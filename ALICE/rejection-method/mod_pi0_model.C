...
void mod_pi0_model() {
  ...
  TH2D *_h_true_CB1=(TH2D*)_l->FindObject("True_pi0_Inv_mass");
...
//  Int_t bins = 27;
  ...
  TH1D* signal_both = new TH1D("signal/background both "," both", bins, pTbins);
  TH1D* signal_true = new TH1D("signal/background true "," true", bins, pTbins);
  ...
  TH1D* signal_bg_true_cb = new TH1D("signal/background bg true cb"," true cb", bins, pTbins);

  TH1D* signal_all_cb = new TH1D("signal/background all cb"," all cb", bins, pTbins);
  TH1D* signal_both_cb = new TH1D("signal/background both cb"," Disp&CPV cb", bins, pTbins);
  TH1D* signal_true_cb = new TH1D("signal/background true cb","true cb", bins, pTbins);
  ...
  TH1D* pi0_spectrum_true_cb = new TH1D("pi0_spectrum_true cb","pi0 spectrum true cb", bins, pTbins);
  ...
  TH1D* position_true_cb = new TH1D("pi0position_cb true","pi0 position_cb true", bins, pTbins);//11
  TH1D* width_true_cb = new TH1D("pi0width_cb true", "pi0 width cb true", bins, pTbins);//watch the number of bins!
  ...
  for(int i=1; i<bins; i++)
    ...
    //  cout<<"work"<<i<<endl;
      ...
      bin2 = (Int_t) _h_all->GetYaxis()->FindBin(pTbins[i+1]);
          ...
          _hf_true_cb=fill_hist(_h_true, bin1, bin2, "true_cb", hist_name, i);

          _hf_all->SetLineColor(kBlue);
          _hf_both->SetLineColor(kRed);
          _hf_true->SetLineColor(kOrange);
          _hf_all_cb->SetLineColor(kBlack);
          _hf_both_cb->SetLineColor(kGreen);
          _hf_true_cb->SetLineColor(kCyan);
          ...
          _hf_true->SetMarkerColor(kOrange);
          _hf_true_cb->SetMarkerColor(kCyan);
          ...
          c3->cd(i+1);
      //    IntFit(c3,_hf_true, _hf_true_cb, "True", signal_true, signal_bg_true, signal_true_cb, signal_bg_true_cb, dpt, i, pi0_spectrum_true, pi0_spectrum_true_cb, position_true, position_true_cb, width_true, width_true_cb);
        ...
        //  h[i][0]=_hf_true;
        //  h[i][1]=_hf_true_cb;
          beauty_draw(h, i, "True", hist_name);
}
...
//draw_spec(pi0_spectrum_true, pi0_spectrum_true_cb, position_true, position_true_cb, width_true, width_true_cb, "True", events);
...
void IntFit(TCanvas *c, TH1D* _hf, TH1D* _hf_cb, char* name, TH1D* signal, TH1D* signal_bg, TH1D* signal_cb, TH1D* signal_bg_cb, double dpt, int i, TH1D * spectrum, TH1D * spectrum_cb, TH1D* position, TH1D* positionCB, TH1D* width, TH1D* widthCB)
{
  ...
  cout<<"BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB Gaus Integral: "<< num  <<endl;
  cout<<"BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB Gaus Integral error: "<< num_err <<endl;
  ...
  cout<<"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA Signal: "<<signal_v<<endl;
  cout<<"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA Signal_error: "<<signal_v_error<<endl;
  ...
  cout<<"Signal: "<<signal_v_cb<<endl;
  cout<<"Signal_error: "<<signal_v_error_cb<<endl;

  signal_cb->SetBinContent(i+1, signal_v_cb/dpt);//только сигнал
  signal_cb->SetBinError(i+1, signal_v_error_cb/dpt);

  signal_bg->SetBinContent(i+1, (num - signal_v)/dpt);///
  signal_bg->SetBinContent(i+1, signal_v_error/dpt);
  ...
  spectrum_cb->SetBinContent(i+1, signal_v_cb/dpt);
  spectrum_cb->SetBinError(i+1, signal_v_error_cb/dpt);
...
//  TCanvas *c2 = new TCanvas("c2_results","The Fit Canvas",1200,800);
  ...
  pi0spectrum->GetXaxis()->SetRangeUser(0., 25.);
...
cout<<"work!"<<endl;
  c2->SaveAs("pi0/model/"+name+"pi0spectrum_CB1_MC.pdf");
  if(name = "True")
  {
    cout<<"TRUEEEEEEEEEEEEEEEEEE"<<endl;
  }
}
