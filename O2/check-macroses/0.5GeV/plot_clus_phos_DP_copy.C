...
void IntFit(TH1D* _h);
void plot_clus_phos_DP_copy(int ievent, TString inputfile, TProfile *hprof);

void plot_clus_phos_DP_copy()
{
  auto c1 = new TCanvas("c1","Eneregy resolution",200,10,700,500);
  auto hprof  = new TProfile("hprof","Profile of pz versus px",100,-4,4,0,20);
  for(int i = 0; i<5; i++)
  {
    plot_clus_phos_DP_copy(0, "/home/hellaweas/alice/work/all_energies/"+"phosclusters"+Tstring(i)+".root", hprof)
  }
...
//  cluTree->SetBranchAddress("mTruthArray", &clu_energy);
  ...
  for (int m = 0; m < 4; m++)
  {
    energyMod_MC[m] = new TH1D(Form("energyModMC%d", m + 1), Form("energy MC Mod%d", m + 1), 100, 0.2, 0.6);
    energyMod_Rec[m] = new TH1D(Form("energyMod%d", m + 1), Form("energy Rec Mod%d", m + 1), 100, 0.2, 0.6);
  ...
  for (int m = 0; m < 4; m++)
  {
    ...
    if(m==2)
    {
      IntFit(energyMod_Rec[m], hprof, energy_MC);
    }
    canv[m]->cd(2);
...
void IntFit(TH1D* _h, TProfile *hprofm float energy_MC)
{
...
//  cout<<"#############################################Integral error: "<< num_err <<endl;
  ...
  hprof->Fill(sigma/mean, eneregy_MC);
