...
void plot_clus_phos_DP_copy(int ievent = 0, TString inputfile = "phosclusters.root")
{
  ...
  for (int m = 0; m < 4; m++)
  {
    energyMod_MC[m] = new TH1D(Form("energyMod%d", m + 1), Form("energy MC Mod%d", m + 1), 100, 8.9, 10.1);
    energyMod_Rec[m] = new TH1D(Form("energyMod%d", m + 1), Form("energy Rec Mod%d", m + 1), 100, 8.9, 10.1);
...
//  TH1D* Cluster_per_event1=Cluster_per_event->ProjectionX();
  ...
  Cluster_per_event->SaveAs("Cluster_per_event_10GeV.pdf");
}

void IntFit(TH1D* _h)
{
    TF1  *f1 = new TF1("f1","gaus", 8.9, 10.1);
        f1->SetLineStyle(2);
  //  TF1 *Number= new TF1("f2","gaus", 9.3, 10.1);

       f1->SetParLimits(1, 9.5, 9.95);
       f1->SetParLimits(2, 0.01, 0.025);
  //f1->SetParameters(1.*_h->GetBinContent(66),0.44,0.006,1.,3.,2.*_h->GetBinContent(51)) ;
