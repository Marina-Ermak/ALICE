...
//  cluTree->SetBranchAddress("mTruthArray", &clu_energy);
  ...
  for (int m = 0; m < 4; m++)
  {
    energyMod_MC[m] = new TH1D(Form("energyModMC%d", m + 1), Form("energy MC Mod%d", m + 1), 100, 1, 2.5);
    energyMod_Rec[m] = new TH1D(Form("energyMod%d", m + 1), Form("energy Rec Mod%d", m + 1), 100, 1, 2.5);
...
//  TH1D* Cluster_per_event1=Cluster_per_event->ProjectionX();
  ...
  Cluster_per_event->SaveAs("Cluster_per_event_2GeV.pdf");
}


void IntFit(TH1D* _h)
{
  TF1  *f1 = new TF1("f1","gaus", 1.7, 2.1);
      f1->SetLineStyle(2);
//  TF1 *Number= new TF1("f2","gaus", 0.35, 0.55);

     f1->SetParLimits(1, 1.7, 1.9);
     f1->SetParLimits(2, 0.065, 0.08);
