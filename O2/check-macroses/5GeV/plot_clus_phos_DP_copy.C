...
//  cluTree->SetBranchAddress("mTruthArray", &clu_energy);
  ...
  for (int m = 0; m < 4; m++)
  {
    energyMod_MC[m] = new TH1D(Form("energyModMC%d", m + 1), Form("energy MC Mod%d", m + 1), 100, 4., 5.2);
    energyMod_Rec[m] = new TH1D(Form("energyMod%d", m + 1), Form("energy Rec Mod%d", m + 1), 100, 4., 5.2);
...
//  TH1D* Cluster_per_event1=Cluster_per_event->ProjectionX();
  ...
  Cluster_per_event->SaveAs("Cluster_per_event_5GeV.pdf");
}


void IntFit(TH1D* _h)
{
  TF1  *f1 = new TF1("f1","gaus", 4.4, 5.);
      f1->SetLineStyle(2);
