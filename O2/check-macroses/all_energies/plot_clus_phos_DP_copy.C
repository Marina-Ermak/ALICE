...
void IntFit(TH1D* _h, TProfile *hprof, TProfile *hprof2, float energy_MC, Double_t x_1, Double_t x_2);
void plot_clus_phos(int ievent, TString inputfile, TProfile *hprof, TProfile *hprof2, int i, Double_t x_1, Double_t x_2, Double_t MC_energy);

void plot_clus_phos_DP_copy()
{
  TCanvas *c1 = new TCanvas("Eneregy resolution","Eneregy resolution",800,600);
//  c1->Divide(1,2);
  TProfile *hprof  = new TProfile("hprof","Energy resolution",200, 0.0, 10.5, 0., 20.);
  TProfile *hprof2  = new TProfile("hprof2","Linearity",200, 0.0, 10.5, 0., 20.);
  TString name;

  plot_clus_phos(0, "/home/hellaweas/alice/work/all_energies/phosclusters_0.root", hprof, hprof2, 0, 0.2, 0.6, 0.5);
  plot_clus_phos(0, "/home/hellaweas/alice/work/all_energies/phosclusters_1.root", hprof, hprof2, 0, 0.6, 1.1, 1.);
  plot_clus_phos(0, "/home/hellaweas/alice/work/all_energies/phosclusters_2.root", hprof, hprof2, 0, 1., 2.5, 2.);
  plot_clus_phos(0, "/home/hellaweas/alice/work/all_energies/phosclusters_3.root", hprof, hprof2, 0, 4., 5.2, 5.);
  plot_clus_phos(0, "/home/hellaweas/alice/work/all_energies/phosclusters_4.root", hprof, hprof2, 0, 8.9, 10.1, 10.);

  c1->cd(0);
  hprof->SetLineColor(kBlack);
  hprof->SetMarkerStyle(20);
  hprof->GetXaxis()->SetTitle("E_{#gamma}, GeV");
  hprof->GetYaxis()->SetTitle("#sigma/E_{mes}");
  hprof->Draw("pe1");

  c1->SaveAs("Resolution.pdf");

  TCanvas *c2 = new TCanvas("Linearity","Linearity",800,600);

  c2->cd(0);
  hprof2->SetLineColor(kBlack);
  hprof2->SetMarkerStyle(20);
  hprof2->GetXaxis()->SetTitle("E_{#gamma}, GeV");
  hprof2->GetYaxis()->SetTitle("(E_{mes} - E_{#gamma})/E_{#gamma}");
  hprof2->Draw("pe1");

  c2->SaveAs("Linearity.pdf");
...
{
  using namespace std;
  TString kine_name = Form("/home/hellaweas/alice/work/all_energies/o2sim_Kine_%d.root", i);
  cout<<"NAME ___ "<<kine_name;
  TFile *fKine = new TFile(kine_name);
...
//  cout << "Lables " << mClustersMCArray->getNElements() << endl;
  ...
  for (int m = 0; m < 4; m++)
  {
    energyMod_MC[m] = new TH1D(Form("energyModMC%d", m + 1), Form("energy MC Mod%d", m + 1), 100, x_1, x_2);
    energyMod_Rec[m] = new TH1D(Form("energyMod%d", m + 1), Form("energy Rec Mod%d", m + 1), 100, x_1, x_2);
...
//    cout << " --- " << endl;
  ...
  //  Cluster_per_event->Fill(count, mLastClusterInEvent-firstClusterInEvent);
    ...
    //  cout<<"Cluster: x1: "<<x1<<" z1: "<<z1<<" x1_loc: "<<x1_loc<<" z1_loc: "<<z1_loc<<endl;
      ...
      energy_MC=mcTrack.GetEnergy();
    //  cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!! ENREGY MC"<<energy_MC<<endl;
  ...
  for (int m = 0; m < 4; m++)
  {
    ...
    if(m==2)
    {
      IntFit(energyMod_Rec[m], hprof, hprof2, MC_energy, x_1, x_2);
    }
    canv[m]->cd(2);
...
void IntFit(TH1D* _h, TProfile *hprof, TProfile *hprof2, float energy_MC, Double_t x_1, Double_t x_2)
{
  TF1  *f1 = new TF1("f1","gaus", x_1, x_2);
      f1->SetLineStyle(2);
...
//  cout<<"#############################################Integral error: "<< num_err <<endl;
  ...
  cout<<"#############################################sigma/mean "<< sigma/mean <<endl;


  hprof->Fill(energy_MC, sigma/mean);
  hprof2->Fill(energy_MC, abs(mean-energy_MC)/energy_MC);

  cout<<"#############################################(mean-energy_MC)/energy_MC "<< abs(mean-energy_MC)/energy_MC <<endl;
